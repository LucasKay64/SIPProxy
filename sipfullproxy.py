...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def processRegister(self):
        ...
        if expires == 0:
            if fromm in registrar:
                del registrar[fromm]
                self.sendResponse("200 VSETKO 0K")
                return
        else:
            now = int(time.time())
    ...
    def processInvite(self):
        ...
        if len(origin) == 0 or not origin in registrar:
            self.sendResponse("400 (Very) Bad Request")
            return
        destination = self.getDestination()
        if len(destination) > 0:
            #
            logging.info("source (hovor od): %s ---> destination (hovor komu) %s,\t %s" % (origin, destination, self.data[5]))
            #
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse("480 (Unfortunately) Temporarily Unavailable")
        else:
    ...
    def processNonInvite(self):
        ...
        destination = self.getDestination()
        if len(destination) > 0:
            #logging.info("destination (komu) %s" % destination)
            #
            #logging.info("source (od koho) %s" % origin)
            #
            if destination in registrar and self.checkValidity(destination):
                socket, claddr = self.getSocketInfo(destination)
                # self.changeRequestUri()
    ...
    def processCode(self):
        ...
        if len(origin) > 0:
            ...
            if origin in registrar:
                ...
                temp_data = temp_data[0][8:11]
                if temp_data == "200":
                    if last_message == "INVITE":
                        logging.info("%s - hovor bol prijaty" % self.data[5])
                elif temp_data == "603":
                    logging.info("%s - hovor nebol prijaty" % self.data[5])
    ...
    def processRequest(self):
        ...
        if len(self.data) > 0:
            ...
            if rx_register.search(request_uri):
                ...
                self.sendResponse("200 VSETKO 0K")
                last_message = "SUBSCRIBE"
            elif rx_publish.search(request_uri):
                self.sendResponse("200 VSETKO 0K")
                last_message = "PUBLISH"
